  ...
  while (offset < length) {
    if (this.state === s.clean) {
      ...
      if (byte >= 208) {
        ...
        this.state = byte >= 200 ? s.end_tckt : s.canc_tckt;
        this._reverse = (byte & 4) !== 0;
        this._reference_s = byte & 3;
      }

      else if (byte >= 184) {
        this.state = byte >= 188 ? s.buf_tckt : s.msg_tckt;
        this._size_s = byte & 3;
      }

      else if (byte === 183) { this.state = s.ack_tckt; }
      else if (byte === 182) { this.state = s.resd_tckt; }

      else if (byte === 181) { this.state = s.resm_hex; }
      else if (byte === 180) { this.state = s.estd_hex; }
      else if (byte === 179) { this.state = s.sltn_hex; }
      else if (byte === 178) { this.state = s.chlng_hex; }

      else if (byte === 3) { this.trigger('disconnect'); continue; }

      else if (byte === 2) { this.state = s.abort_errno; }
      else if (byte === 1) { this.state = s.auth_revision; }
      else if (byte === 0) { this.state = s.conn_revision; }
