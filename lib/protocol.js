...
function Interpreter(socket) {
  ...
  socket.on('data', function(chunk) {
    if (!self.disabled) self.parse(chunk);
  });
...
Interpreter.prototype.send = function(method, params) {
  var ticket, ref, buffer, size, reverse, version, hex, o, s,
      m = -1,
      socket = this.socket;
  ...
  switch (method) {
    ...
    case 'buffer':  if (!~m) m = 188;
      ...
      if (s <= 0xff) {
        socket.write(new Buffer([
          m, ticket / 0x100 & 0xff, ticket & 0xff, size
        ]));
      } else if (s <= 0xffff) {
        socket.write(new Buffer([
          m | 1, ticket / 0x100 & 0xff, ticket & 0xff, s / 0x100 & 0xff,
          s & 0xff
        ]));
      } else if (s <= 0xffffff) {
        socket.write(new Buffer([
          m | 2, ticket / 0x100 & 0xff, ticket & 0xff, size / 0x10000 & 0xff,
          size / 0x100 & 0xff, size & 0xff
        ]));
      } else {
        socket.write(new Buffer([
          m | 3, ticket / 0x100 & 0xff, ticket & 0xff, size / 0x1000000 & 0xff,
          size / 0x10000 & 0xff, size / 0x100 & 0xff, size & 0xff
        ]));
      }
    ...
    case 'feed':   if (!~m) m = 240;
      ...
      o = [m, ticket / 0x100 & 0xff, ticket & 0xff];

      if (ref <= 0xff) {
        o.push(ref);
      } else if (ref <= 0xffff) {
        o[0] |= 1 << 2;
        o.push(ref / 0x100 & 0xff);
        o.push(ref & 0xff);
      } else if (ref <= 0xffffff) {
        o[0] |= 2 << 2;
        o.push(ref / 0x10000 & 0xff);
        o.push(ref / 0x100 & 0xff);
        o.push(ref & 0xff);
      } else {
        o[0] |= 3 << 2;
        o.push(ref / 0x1000000 & 0xff);
        o.push(ref / 0x10000 & 0xff);
        o.push(ref / 0x100 & 0xff);
        o.push(ref & 0xff);
      }
      ...
      if (size <= 0xff) {
        ...
        o[0] |= 1;
        o.push(size / 0x100 & 0xff);
        o.push(size & 0xff);
      } else if (size <= 0xffffff) {
        o[0] |= 2;
        o.push(size / 0x10000 & 0xff);
        o.push(size / 0x100 & 0xff);
        o.push(size & 0xff);
      } else {
        o[0] |= 3;
        o.push(size / 0x1000000 & 0xff);
        o.push(size / 0x10000 & 0xff);
        o.push(size / 0x100 & 0xff);
        o.push(size & 0xff);
      }
    ...
    case 'acknowledge': if (!~m) m = 183;
      ...
      ticket = params[0];
      this.socket.write(new Buffer([m, ticket / 0x100 & 0xff, ticket & 0xff]));
      break;
    ...
    case 'end':    if (!~m) m = 200;
      ...
      if (ref <= 0xff) {
        socket.write(new Buffer([
          m, ticket / 0x100 & 0xff, ticket & 0xff, ref
        ]));
      } else if (ref <= 0xffff) {
        socket.write(new Buffer([
          m | 1, ticket / 0x100 & 0xff, ticket & 0xff, ref / 0x100 & 0xff,
          ref & 0xff
        ]));
      } else if (ref <= 0xffffff) {
        socket.write(new Buffer([
          m | 2, ticket / 0x100 & 0xff, ticket & 0xff, ref / 0x10000 & 0xff,
          ref / 0x100 & 0xff, ref & 0xff
        ]));
      } else {
        socket.write(new Buffer([
          m | 3, ticket / 0x100 & 0xff, ticket & 0xff, ref / 0x1000000 & 0xff,
          ref / 0x10000 & 0xff, ref / 0x100 & 0xff, ref & 0xff
        ]));
      }
    ...
    case 'abort':        if (!~m) m = 2;
      ...
      version = params[0];
      socket.write(new Buffer([m, version]));
      break;
    ...
    case 'resume':      if (!~m) m = 181;
      ...
      if (method === 'resume') {
        ticket = params[1];
        socket.write(new Buffer([ticket/0x100&0xff, ticket&0xff]));
      }
