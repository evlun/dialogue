...
Handle.prototype.send = function(method, a, b, c) {
  ...
  switch (method) {
    ...
    case 'message':
      ...
      if (s <= 0xff) {
        socket.write(new Buffer([180, a/0x100&0xff, a&0xff, s]));
      } else if (s <= 0xffff) {
        socket.write(new Buffer([180 | 1, a/0x100&0xff, a&0xff,
          s/0x100&0xff, s&0xff]));
      } else if (s <= 0xffffff) {
        socket.write(new Buffer([180 | 2, a/0x100&0xff, a&0xff,
          s/0x10000&0xff, s/0x100&0xff, s&0xff]));
      } else {
        socket.write(new Buffer([180 | 3, a/0x100&0xff, a&0xff,
    ...
    case 'challenge':   m = 170;
    case 'solution':    if (!~m) m = 171;
    case 'established': if (!~m) m = 172;
    case 'resume':      if (!~m) m = 173;
