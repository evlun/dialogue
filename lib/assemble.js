function assemble(stream, method, params) {
  ...
  switch (method) {
    ...
    case 'established': m = 3;
    case 'resume':      if (!~m) m = 4;
    case 'challenge':   if (!~m) m = 5;
    case 'solution':    if (!~m) m = 6;
    ...
    case 'restored':   m = 7;
    case 'disconnect': if (!~m) m = 8;
    case 'destroy':    if (!~m) m = 9;
    ...
    case 'acknowledge':
      ...
      t = params[0];
      ret = stream.write(new Buffer([10, t / 0x100 & 0xff, t & 0xff]));
      break;
